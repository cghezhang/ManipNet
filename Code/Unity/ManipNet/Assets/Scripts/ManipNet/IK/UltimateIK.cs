...
public class UltimateIK : MonoBehaviour {
    ...
    public enum JOINT {Free, HingeX, HingeY, HingeZ, Ball, HingeYZ}
    ...
    public class Model {
        ...
        public void Solve() {
            DateTime timestamp = Extensions.GetTimestamp();
            if(IsSetup()) {
                ComputeLevels();
                if(SeedZeroPose) {
                    foreach(Bone bone in Bones) {
            ...
            SolveTime = (float)Extensions.GetElapsedTime(timestamp);
        }
    ...
    public class Bone {
        ...
        // this is for joint that has 2DOFs
        public float anotherLowerLimit = 0f; 
        public float anotherUpperLimit = 0f; 
        ...
        public void SetJoint(JOINT joint) {
            if(Joint != joint) {
                ...
                LowerLimit = 0f;
                UpperLimit = 0f;
                anotherLowerLimit = 0f;
                anotherUpperLimit = 0f;
            }
        }
        ...
        public void ClampLimits() {
            ...
            anotherLowerLimit = Mathf.Clamp(anotherLowerLimit, -180f, 0f);
            anotherUpperLimit = Mathf.Clamp(anotherUpperLimit, 0f, 180f);
        }
		...
		public void ResolveLimits(bool avoidLocalOptima) {
            switch(Joint) {
                ...
                case JOINT.HingeYZ:
                {
                    float LowerLimitY = LowerLimit;
                    float UpperLimitY = UpperLimit;

                    float LowerLimitZ = anotherLowerLimit;
                    float UpperLimitZ = anotherUpperLimit;

                    // first get hinge Y
                    Quaternion currentLocalRotation = ZeroRotation;
                    Quaternion targetLocalRotation = Transform.localRotation;
                    float angle = Vector3.SignedAngle(currentLocalRotation.GetRight(), Vector3.ProjectOnPlane(targetLocalRotation.GetRight(), currentLocalRotation.GetUp()), currentLocalRotation.GetUp());
                    currentLocalRotation = currentLocalRotation * (avoidLocalOptima ?
                    Quaternion.AngleAxis(Mathf.Repeat(angle-LowerLimitY, UpperLimitY-LowerLimitY) + LowerLimitY, Vector3.up):
                    Quaternion.AngleAxis(Mathf.Clamp(angle, LowerLimitY, UpperLimitY), Vector3.up));

                    // then get hinge z
                    angle = Vector3.SignedAngle(currentLocalRotation.GetUp(), Vector3.ProjectOnPlane(targetLocalRotation.GetUp(), currentLocalRotation.GetForward()), currentLocalRotation.GetForward());
                    currentLocalRotation = currentLocalRotation * (avoidLocalOptima ?
                    Quaternion.AngleAxis(Mathf.Repeat(angle-LowerLimitZ, UpperLimitZ-LowerLimitZ) + LowerLimitZ, Vector3.forward):
                    Quaternion.AngleAxis(Mathf.Clamp(angle, LowerLimitZ, UpperLimitZ), Vector3.forward));
	...
	public class UltimateIK_Editor : Editor {
        ...
        void SetObjective(int index, Transform t) {
            ...
            if(t != null) {
                if(Target.Objectives.Contains(t) || !IsInsideHierarchy(Target.transform, t)) {
                    return;
                }
            }
		...
		public override void OnInspectorGUI() {
            ...
            Extensions.SetGUIColor(Background);
            using(new EditorGUILayout.VerticalScope ("Box")) {
                Extensions.ResetGUIColor();

                Extensions.SetGUIColor(Header);
                using(new EditorGUILayout.VerticalScope ("Box")) {
                    Extensions.ResetGUIColor();
                    EditorGUILayout.LabelField("Setup");
                }
                Extensions.SetGUIColor(Content);
                using(new EditorGUILayout.VerticalScope ("Box")) {
                    Extensions.ResetGUIColor();
                    ...
                    if(Extensions.GUIButton("Override", RegularButton, RegularFont, 100f, 18f)) {
                        Target.Skeleton.SaveAsZeroPose();
                    }
                    EditorGUILayout.EndHorizontal();
                    SetRoot((Transform)EditorGUILayout.ObjectField("Root", Target.Root, typeof(Transform), true));
                    EditorGUILayout.BeginHorizontal();
                    if(Extensions.GUIButton("Root Translation X", Target.Skeleton.RootTranslationX ? ActiveButton : PassiveButton, Target.Skeleton.RootTranslationX ? ActiveFont : PassiveFont)) {
                        Target.Skeleton.RootTranslationX = !Target.Skeleton.RootTranslationX;
                    }
                    if(Extensions.GUIButton("Root Translation Y", Target.Skeleton.RootTranslationY ? ActiveButton : PassiveButton, Target.Skeleton.RootTranslationY ? ActiveFont : PassiveFont)) {
                        Target.Skeleton.RootTranslationY = !Target.Skeleton.RootTranslationY;
                    }
                    if(Extensions.GUIButton("Root Translation Z", Target.Skeleton.RootTranslationZ ? ActiveButton : PassiveButton, Target.Skeleton.RootTranslationZ ? ActiveFont : PassiveFont)) {
            ...
            using(new EditorGUILayout.VerticalScope ("Box")) {
                ...
                using(new EditorGUILayout.VerticalScope ("Box")) {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    if(Extensions.GUIButton("Add Objective", RegularButton, RegularFont)) {
            ...
            using(new EditorGUILayout.VerticalScope ("Box")) {
                Extensions.ResetGUIColor();
                if(Extensions.GUIButton(Target.Draw ? "Drawing On" : "Drawing Off", Target.Draw ? ActiveButton : PassiveButton, Target.Draw ? ActiveFont : PassiveFont)) {
        ...
        void InspectBone(Bone bone) {
            ...
            using(new EditorGUILayout.VerticalScope ("Box")) {
                ...
                if(bone.Index == Target.SelectedBone) {
                    if(!bone.Active || Extensions.GUIButton(bone.Transform.name, ActiveButton, ActiveFont)) {
                        ResetPreview();
                        Target.SelectedBone = -1;
                    }

                    Extensions.SetGUIColor(Panel);
                    using(new EditorGUILayout.VerticalScope ("Box")) {
                        Extensions.ResetGUIColor();
                    ...
                    using(new EditorGUILayout.VerticalScope ("Box")) {
                        ...
                        if(Extensions.GUIButton("Show Preview", Target.ShowPreview ? ActiveButton : PassiveButton, Target.ShowPreview ? ActiveFont : PassiveFont)) {
                    ...
                    if(Extensions.GUIButton(bone.Transform.name, RegularButton, RegularFont)) {
                        ResetPreview();
        ...
        void InspectObjective(int index) {
            if(Target.Objectives[index] != null && Target.Skeleton.FindObjective(Target.Objectives[index]) != null) {
                ...
                using(new EditorGUILayout.VerticalScope ("Box")) {
                    ...
                    EditorGUI.BeginDisabledGroup(true);
                    Extensions.SetGUIColor(ValidField);
                    EditorGUILayout.ObjectField(Target.Objectives[index], typeof(Transform), true);
                    Extensions.ResetGUIColor();
                    EditorGUI.EndDisabledGroup();
                    if(Extensions.GUIButton("X", InvalidField, RegularFont, 36f, 18f)) {
                        ...
                        if(Extensions.GUIButton("Solve Position", o.SolvePosition ? ActiveButton : PassiveButton, o.SolvePosition ? ActiveFont : PassiveFont)) {
                            o.SolvePosition = !o.SolvePosition;
                        }
                        if(Extensions.GUIButton("Solve Rotation", o.SolveRotation ? ActiveButton : PassiveButton, o.SolveRotation ? ActiveFont : PassiveFont)) {
                ...
                EditorGUILayout.BeginHorizontal();
                Extensions.SetGUIColor(InvalidField);
...
public static class Extensions {
	...
	// }
	// public static T Add<T>(ref T[] array, T element) {
	// 	Expand(ref array);
