...
namespace DeepLearning {
    ...
    public class Matrix {
        ...
        public Matrix CopyTo(Matrix newMatrix){
            if(newMatrix.GetCols() == GetCols() && newMatrix.GetRows() == GetRows()){
                Eigen.Scale(Ptr, 1f, newMatrix.Ptr);
            }
            else
            {
                Debug.Log("Copy Matrix Wrong with sizes " + " Origin " + GetRows() + " " + GetCols() + 
                               " and Target " + newMatrix.GetRows() + " " + newMatrix.GetCols());   
            }
            return newMatrix;
        }
