...
public class MotionData : ScriptableObject {
	...
	// we save the objectWorld as array in Frame, so we need an order such that although the object orders are changed in the scene
	public string[] sceneObjectNames = new string[0];
	
	// hard code for getting hand rest pose, two hands flatten here
	public Matrix4x4[] restPose = new Matrix4x4[0];
	public Matrix4x4[] restLocalPose = new Matrix4x4[0];
	// hard code for getting dof here
	public int[] Dofs = new int[0];
	// hard code for getting hand rest pose, two hands flatten here
	...
	// ============ for Rest Pose ===============
	public Matrix4x4[] GetRestPose(int handIndex){
		int numBones = restPose.Length/2;
		Matrix4x4[] restTrans = new Matrix4x4[numBones];
		for(int i=0; i<numBones; i++){
			restTrans[i] = restPose[handIndex*numBones+i];
		}
		return restTrans;
	}
	...
	public GameObject GetSceneObject(int iObj){
		Scene scene = GetScene();
		GameObject obj = scene.GetRootGameObjects()[0].transform.GetChild(iObj).gameObject;
		if(obj.name != sceneObjectNames[iObj]){
			Debug.Log("scene object name should be: " + sceneObjectNames[iObj] + " but current object is:" + obj.name);
			Debug.LogError("finally wrong here!! you need to implement searching by name!");
		}
		return obj;
	}
	...
	public GameObject[] GetSceneObjects(){
		...
		foreach(GameObject instance in scene.GetRootGameObjects()) {
			for(int iObj=0; iObj<instance.transform.childCount; iObj++){
				GameObject obj = instance.transform.GetChild(iObj).gameObject;
				// if(obj.name != sceneObjectNames[iObj]){
				// 	Debug.LogError("finally wrong here!! you need to implement searching by name!");
				// }
				listObjects.Add(obj);
	...
	// for hand
	public void CreateScene(GameObject[] objects) {
		if(objects.Length>0){
			UnityEngine.SceneManagement.Scene active = EditorSceneManager.GetActiveScene();
			UnityEngine.SceneManagement.Scene scene = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Additive);
			EditorSceneManager.SetActiveScene(scene);
			...
			GameObject empty = new GameObject("Scene");
			empty.transform.position = Vector3.zero;
			empty.transform.rotation = Quaternion.identity;
			empty.transform.localScale = Vector3.one;
			// resize the object names
			sceneObjectNames = new string[objects.Length];
			for(int i=0; i<objects.Length; i++){
				objects[i].transform.parent = empty.transform;
				sceneObjectNames[i] = objects[i].name; // this is for fixing the order
			}
