...
def Normalize_Until(X, N, unitilDim, savefile=None):
    mean = N[0]
    std = N[1]
    originalLen = len(mean)
    mean = mean[:unitilDim]
    std = std[:unitilDim]
    for i in range(std.size):
        if std[i] == 0:
    ...
    mean = np.concatenate((mean, np.zeros(originalLen - unitilDim, dtype=np.float32)), 0)
    std = np.concatenate((std, np.ones(originalLen - unitilDim, dtype=np.float32)), 0)
...
def Normalize_01(X, axis=0, savefile=None):
    # normalize everything into 0-1
    max_each_column = np.max(X, axis=axis)
    min_each_column = np.min(X, axis=axis)

    mean = min_each_column
    std = max_each_column - mean
    for i in range(std.size):
        if std[i] == 0:
            std[i] = 1
            mean[i] = 0
...
""" FOR GET A BATCH CONTINUOUS INPUI """
...
def ClosestPointOnLineSegment(seg_s, seg_e, p):
    ...
    newP = seg_s + t * span
    return newP, np.linalg.norm(newP - p, axis=-1)
